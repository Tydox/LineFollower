...
void update_Color()
{
  ...
  {    musika();  }//after reading the color, call function to make a buzzing sound
}
//set music-------------------------------------------------------------------------------------------------------------
void musika()
{
...
bool sharpturn()
{
  if (error == -20 || error == 20)  //wait split second to check again & make sure the error is indeed a 90 degree turn
  {
      time_hold = millis();
    while (millis() > time_hold +40) {
      getLinePositionNum();
      getError();
      if (error == 777) { //full blackline detected, set motors to go straight and exit function
...
void loop()
{
  ...
  getLinePositionNum(); //get the line position from the IR sensors XXXXXXX
  getError(); //set error for pid
  if(!sharpturn()){//check for 90d turns
  set_motors(); //set speed accordng to PID
    }
  }
}
